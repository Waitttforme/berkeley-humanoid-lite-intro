...
const nav = [['overview', '数字机器人'], ['actuators', '智能关节'], ['solution', '技术方案'], ['demo', '交互演示'], ['evidence', '实物证据'], ['outcomes', '应用与成果']]
...
function JointModels() {
  const joints = [
    {
      code: '5010',
      className: 'joint-5010',
      image: 'media/joints/actuator-5010.png',
      title: '紧凑型摆线关节',
      text: '紧凑壳体整合电机、编码反馈与摆线传动，适合空间受限的肢体节点；可拆端盖和标准连接件让装配、替换与维护更直接。',
      facts: [['结构定位', '紧凑关节节点'], ['制造适配', '多种打印壳体'], ['连接方式', '驱动与反馈一体']],
    },
    {
      code: '6512',
      className: 'joint-6512',
      image: 'media/joints/actuator-6512.png',
      title: '高承载摆线关节',
      text: '更大的承载结构整合输入轴、输出轴与摆线盘，面向髋部、膝部等主要受力位置；完整装配关系便于定位传动和维护路径。',
      facts: [['结构定位', '高负载关节层'], ['装配组成', '输入轴 + 输出轴'], ['工程表达', '整机装配与清单']],
    },
  ]
  return <section className="section actuator-models" id="actuators">
    <Heading index="01" label="CONNECTED ACTUATORS" title="两种关节，构成机器人的执行网络" text="机械关节同时承担动作执行与状态反馈，是整机物联网中可寻址、可诊断的现场节点。"/>
    <div className="actuator-model-grid">
      {joints.map(joint => <article className={`actuator-model-card ${joint.className}`} key={joint.code}>
        <div className="actuator-model-visual"><span>{joint.code}</span><img src={asset(joint.image)} alt={`${joint.code} 摆线关节三维建模`} loading="lazy" decoding="async"/></div>
        <div className="actuator-model-copy"><small>{joint.code} / ACTUATOR NODE</small><h3>{joint.title}</h3><p>{joint.text}</p><dl>{joint.facts.map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value}</dd></div>)}</dl></div>
      </article>)}
    </div>
    <div className="actuator-loop" aria-label="智能关节工作闭环"><span><b>01</b>位置与状态感知</span><i>→</i><span><b>02</b>节点闭环控制</span><i>→</i><span><b>03</b>CAN 指令与反馈</span><i>→</i><span><b>04</b>诊断与服务记录</span></div>
  </section>
}
...
const reportSections = [
  ...
  ['08 / 交付与后续路线', ['当前交付为公网可访问的软件演示系统、本技术报告、可交互的原有技术展厅以及可导出的模拟服务记录。静态网站可通过 GitHub Pages 分发，业务逻辑在浏览器运行。', '后续顺序：单机只读遥测接入 → 规则标定与实测验证 → 服务端工单与权限 → 多设备试点。真实任务控制需要独立设计授权、确认、状态反馈与本体安全约束，不应由本演示按钮直接承担。']],
]
...
export default function CompetitionApp() {
  ...
  return <div className="competition"><a className="skip-link" href="#demo">跳到交互演示</a><header className="site-header"><a className="brand" href="#overview"><span className="brand-symbol">H<span>·</span></span><span>HUMANOID<small>SMART SERVICE SYSTEM</small></span></a><nav className={menu ? 'open' : ''} aria-label="主要导航">{nav.map(([id, label]) => <a key={id} className={active === id ? 'active' : ''} href={`#${id}`} onClick={() => setMenu(false)}>{label}</a>)}</nav><a className="header-cta" href="#demo">进入演示<ArrowUpRight size={15}/></a><button className="menu-toggle" aria-label={menu ? '关闭导航' : '打开导航'} aria-expanded={menu} onClick={() => setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button></header>
    ...
    <JointModels/>
    <section className="section" id="solution"><Heading index="02" label="SYSTEM ARCHITECTURE" title="端到边的系统协同" text="以设备状态为共同语言，把感知、控制、通信和边缘处理组织成可理解的系统。"/><SystemOverview/><div className="engineering-workbench"><div className="workbench-heading"><div><span className="workbench-dot" aria-hidden="true"/><strong>系统分层详解</strong></div><span>架构说明 · 非实时链路</span></div><div className="architecture" role="group" aria-label="技术方案分层导航">{layers.map((l, i) => <button key={l.en} className={i === layer ? 'active' : ''} aria-pressed={i === layer} onClick={() => setLayer(i)}><span>0{i + 1}<l.icon size={23}/></span><strong>{l.name}</strong><small>{l.en}</small>{i < 4 && <ChevronRight className="layer-arrow" size={18}/>}</button>)}</div><div className="architecture-detail" key={current.en}><div><span className="pill">{current.status}</span><h3>{current.name}</h3><p>{current.detail}</p></div><dl><div><dt>INPUT / 输入</dt><dd>{current.input}</dd></div><div><dt>OUTPUT / 输出</dt><dd>{current.output}</dd></div></dl></div><LayerDeepDive index={layer}/></div><div className="boundary-note"><CircleAlert size={17}/><p>当前服务端点使用浏览器模拟数据。本体 → 浏览器的遥测适配尚未接入；IMU 通过独立 USB 支路汇入机载计算，不经过关节控制器。</p><a href="#demo">查看服务闭环<ArrowUpRight size={15}/></a></div></section>
    <section className="section demo-section" id="demo"><Heading index="03" label="LIVE SERVICE DEMO" title="让异常处理形成闭环" text="亲手操作一次巡检：发现问题、暂停任务、完成维护，再用复检确认服务结果。"/><ServiceDemo/></section>
    <section className="section" id="innovation"><Heading index="04" label="INNOVATION BY DESIGN" title="连接状态与服务的关键设计" text="从本体技术到智慧服务，分别说明问题、机制、价值与验证方法。点击条目展开细节。"/><InnovationStudy/></section>
    <section className="section evidence-section" id="evidence"><Heading index="05" label="ENGINEERING EVIDENCE" title="从关节建模走向整机实物" text="沿着零件、关节、肢体到整机的顺序查看制造记录，并与首屏数字机器人逐项对照。"/><div className="evidence-grid">{evidence.map((e) => <button className="evidence-card" key={e[0]} onClick={event => { photoTrigger.current = event.currentTarget; setPhoto(e) }}><div className="evidence-image"><img src={asset(`media/project-evidence/${e[0]}-720.webp`)} alt={e[2]} srcSet={`${asset(`media/project-evidence/${e[0]}-720.webp`)} 720w, ${asset(`media/project-evidence/${e[0]}-1280.webp`)} 1280w`} sizes="(max-width: 650px) 90vw, (max-width: 1000px) 44vw, 30vw" loading="lazy" decoding="async" width="720" height="540"/><span>查看实物细节 <ArrowUpRight size={15}/></span></div><small>{e[1]}</small><h3>{e[2]}</h3><p>{e[3]}</p></button>)}</div></section>
    <section className="section" id="outcomes"><Heading index="06" label="APPLICATION & VALUE" title="面向真实场景的持续服务" text="先讲透实验室场景，再以真实接入和验证数据推进教学、研发与设备运维应用。"/><div className="application-layout"><div className="case-tabs" role="group" aria-label="应用场景">{['实验室设备巡检', '教学与实训', '研发与设备运维'].map((title, i) => <button key={title} aria-pressed={useCase === i} className={useCase === i ? 'active' : ''} onClick={() => setUseCase(i)}><span>0{i + 1}</span>{title}<ArrowUpRight size={18}/></button>)}</div><article className="case-detail" key={useCase}><span className="pill">{caseInfo[3]}</span><h3>{caseInfo[1]}</h3><p>{caseInfo[2]}</p><div><span>服务价值</span><strong>{caseInfo[4]}</strong></div></article></div><div className="value-grid"><div><span>目标用户</span><h3>实验室 · 实训中心 · 研发团队</h3><p>围绕设备管理、调试与培训验证需求。</p></div><div><span>交付路径</span><h3>软件演示 → 实机适配 → 试点</h3><p>逐步补齐遥测、规则标定与服务端能力。</p></div><div><span>产业价值假设</span><h3>减少遗漏，提升维护可追溯性</h3><p>以对照实验验证收益，不预设商业数据。</p></div></div><div className="deliverables"><div><span className="eyebrow">PROJECT DELIVERABLES</span><h2>演示与项目成果</h2><p>技术报告覆盖问题背景、技术方案、创新设计、关键技术、应用场景、市场前景与产业价值。</p></div><div className="deliverable-links"><a href="?view=report"><FileText/><span><strong>项目技术报告</strong><small>在线阅读 · 打印 / 保存为 PDF</small></span><ArrowUpRight/></a><a href="#overview"><Box/><span><strong>交互数字机器人</strong><small>结构展开 · 六组动作 · 视角控制</small></span><ArrowUpRight/></a><a href="#demo"><Play/><span><strong>现场演示与服务记录</strong><small>完整操作流程 · JSON 导出</small></span><ArrowUpRight/></a></div></div></section>
