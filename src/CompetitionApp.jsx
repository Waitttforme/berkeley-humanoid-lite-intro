...
export default function CompetitionApp() {
  ...
  return <div className="competition"><a className="skip-link" href="#demo">跳到交互演示</a><header className="site-header"><a className="brand" href="#overview"><span className="brand-symbol">H<span>·</span></span><span>HUMANOID<small>SMART SERVICE SYSTEM</small></span></a><nav className={menu ? 'open' : ''} aria-label="主要导航">{nav.map(([id, label]) => <a key={id} className={active === id ? 'active' : ''} href={`#${id}`} onClick={() => setMenu(false)}>{label}</a>)}</nav><a className="header-cta" href="#demo">进入演示<ArrowUpRight size={15}/></a><button className="menu-toggle" aria-label={menu ? '关闭导航' : '打开导航'} aria-expanded={menu} onClick={() => setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button></header>
    ...
    <section className="section" id="solution"><Heading index="01" label="SYSTEM ARCHITECTURE" title="端到边的系统协同" text="以设备状态为共同语言，把感知、控制、通信和边缘处理组织成可理解的系统。"/><SystemOverview/><div className="architecture">{layers.map((l, i) => <button key={l.en} className={i === layer ? 'active' : ''} aria-pressed={i === layer} onClick={() => setLayer(i)}><span>0{i + 1}<l.icon size={23}/></span><strong>{l.name}</strong><small>{l.en}</small>{i < 4 && <ChevronRight className="layer-arrow" size={18}/>}</button>)}</div><div className="architecture-detail" key={current.en}><div><span className="pill">{current.status}</span><h3>{current.name}</h3><p>{current.detail}</p></div><dl><div><dt>INPUT / 输入</dt><dd>{current.input}</dd></div><div><dt>OUTPUT / 输出</dt><dd>{current.output}</dd></div></dl></div><LayerDeepDive index={layer}/><div className="boundary-note"><CircleAlert size={17}/><p>当前服务端点使用浏览器模拟数据。本体 → 浏览器的遥测适配尚未接入；IMU 通过独立 USB 支路汇入机载计算，不经过关节控制器。</p><a href="?view=technical#iot-showcase">查看详细链路<ArrowUpRight size={15}/></a></div></section>
