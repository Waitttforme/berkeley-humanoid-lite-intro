...
主页在问题背景之后、技术方案之前新增 `#model-studio` 数字样机区。复用交接包动作编排、整机/侧向/拆解渲染图以及 26 个压缩网格；已有原始网格经逐文件哈希比对一致，不重复引入。支持待机、招手、下蹲、步行、姿态展示、复位、展开重组与视角复位。模型点击后加载，手机单指滚动页面、双指操作模型。

交接包 SHA256 校验与复用记录见 `HANDOFF-INTEGRATION.md`。动作均为程序化示意，脚底补偿属于几何处理，不是动力学仿真。可运行 `node scripts/studio-qa.mjs` 验证模型与交互。

### 技术方案与创新设计详解

技术方案新增双支路系统图（关节 CAN 与独立 USB IMU）、通信/策略/网页采样频率对照，以及五层机制、服务映射与接入边界。创新设计提供两类可展开内容：本体技术基础和本次智慧服务实现；每项按问题、机制、价值、验证组织，并附软件与实机验证状态表。报告同步增加技术补充。

技术依据来自系统设计论文、FOC 说明、机载接口与软件开发文档，页面内可核对对应段落。论文实验和网页模拟均明确标识，不将平台既有设计标记为软件原创，也不把论文数据记作本项目实测。此前删除的构建、来源终章与首页运动影像保持移除。

### 展示与动效

新版采用石墨灰背景、柔和白色与薄荷色点缀，字体优先使用设备上的苹方并配置本地许可字体回退。首屏标题分句入场，精细指针设备支持小幅机器人视差；触屏保持静态。滚动揭示使用原生 IntersectionObserver，不接管滚动。服务步骤光点与异常光晕仅在演示可见时运行，页面进入后台时暂停装饰动画。系统“减少动态效果”设置会关闭这些动效，并直接展示全部内容。

`npm run qa:premium` 检查字体实际渲染、文字裁切、多尺寸布局、视差与触屏差异、减少动态效果、离屏动画暂停及技术报告入口。

公网地址：https://waitttforme.github.io/berkeley-humanoid-lite-intro/

首页现为「人形机器人智慧运维与任务服务系统」，包含项目总览、问题背景、五层技术方案、六步交互演示、创新设计、实物证据、应用价值和项目成果。深色工业界面适配桌面与手机。

- `?view=report`：完整在线技术报告，可通过浏览器打印保存；明确实现范围与待验证的实机实验。
- `#model-studio`：主页面内置 URDF/STL 交互数字样机；原独立技术展厅入口已移除。
- 交互演示：温升、通信、姿态三类异常；常规巡检另有无异常归档路径。
- 维护流程：启动 → 注入异常 → 暂停任务 → 生成工单 → 完成检查项 → 复检 → 归档恢复任务。故障样本复检失败时保持工单打开，禁止归档。
- 自动演示可暂停和继续；手动重置会清理当前流程，保留最近归档。
- 数据为浏览器确定性模拟，每秒采样，趋势只绘制已产生样本。诊断使用右膝及全机样本，点击关节可查看相应模拟状态。
- JSON 导出包含数据来源、规则版本和事件链；本机最近五条归档保存在 localStorage，不提供跨设备同步。

验证：`npm test` 检查业务约束；`npm run qa:visual` 检查生产子路径、三种异常、复检失败与恢复、导出、自动演示、记录持久化、320/390/900/1440 像素布局、报告与数字样机。推送 main 后 GitHub Actions 自动发布。
