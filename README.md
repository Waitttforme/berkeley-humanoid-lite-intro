...
- `?view=technical`：保留原技术展厅、按需加载的 URDF/STL 交互样机和既有资料。
...
验证：`npm test` 检查业务约束；`npm run qa:visual` 检查生产子路径、三种异常、复检失败与恢复、导出、自动演示、记录持久化、320/390/900/1440 像素布局、报告与技术入口。`npm run qa:technical` 检查保留的技术展厅。推送 main 后 GitHub Actions 自动发布。
...
技术展厅的“观看约 55 秒系统展演”进入七章自动讲解流程，依次展示平台定位、信号链路、智慧服务、数字展品、实机证据、系统解剖与软件栈。原第八部分“构建”和第九部分“来源”及对应导航已移除；模型与素材许可说明继续保留。
...
截图所示构建教程（BOM、打印、装配、刷写、训练部署及其扩展内容）已从技术展厅移除。
