...
try {
  ...
  for (const [id, label] of [['wave', '招手'], ['squat', '下蹲'], ['walk', '步行'], ['combat', '战斗'], ['attention', '立正']]) {
    ...
    assert.ok((await page.locator('.twin-motion-list button.is-active').innerText()).includes(label))
  }
  await page.getByRole('button', { name: /探索结构/ }).click()
  await page.locator('.main-showcase.is-exploded').waitFor({ state: 'visible' })
  await page.waitForTimeout(1200)
  await page.getByRole('button', { name: /重新组装/ }).click()
  await page.waitForFunction(() => !document.querySelector('.main-showcase')?.classList.contains('is-exploded'))
