...
try {
  ...
  const fontUsage = {}
  for (const selector of ['.main-showcase h1', '.twin-copy>p', '.twin-eyebrow', '.metric-card strong']) {
  ...
  passed.push('report navigation returns to unified 3D homepage')
  ...
  passed.push('architecture detail transition updates')
  ...
  assert.equal(await warning.evaluate(el => getComputedStyle(el, '::before').animationPlayState), 'paused')
  passed.push('offscreen and hidden-document handlers pause warning animation')
  ...
  await touchPage.waitForFunction(() => document.querySelector('.studio-canvas')?.dataset.loaded === 'true', null, { timeout: 120000 })
  assert.equal(await touchPage.locator('.studio-canvas canvas').evaluate(el => el.style.touchAction), 'pan-y')
  await touchPage.close()
  passed.push('touch interaction preserves single-finger page scrolling')
