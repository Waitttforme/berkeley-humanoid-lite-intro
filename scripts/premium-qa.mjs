...
try {
  ...
  assert.equal(await page.locator('.motion-evidence video[preload="none"]').count(), 4)
  passed.push('old motion-film removed; four poster-first self-owned motion records present')
  ...
  assert.equal(await page.locator('a[href*="view=technical"]').count(), 0)
  passed.push('report navigation works and the separate technical exhibition entry is removed')
