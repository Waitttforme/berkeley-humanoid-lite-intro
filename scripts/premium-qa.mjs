...
try {
  ...
  await touchPage.waitForFunction(() => !document.querySelector('.twin-motion-list button')?.disabled, null, { timeout: 120000 })
  assert.equal(await touchPage.locator('.viewer canvas').evaluate(el => el.style.touchAction), 'none')
  await touchPage.close()
  passed.push('touch interaction matches the transferred original model controls')
