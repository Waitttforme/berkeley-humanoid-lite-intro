...
try {
  ...
  // 08–15 / Problem
  await cue({ chapter: '02 / THE CHALLENGE', title: '从设备运行，到服务管理', subtitle: '统一状态、任务、维护和复检记录', truth: '问题背景 / 实验室巡检场景', progress: 9, fullscreen: false })
  ...
  await cue({ chapter: '03 / MOTION STUDIO', title: '姿态切换与关节协同', subtitle: '招手、下蹲、步行和复位均由数字样机实时渲染', truth: '22-DOF MOTION / THREE.JS', progress: 23, fullscreen: false })
  ...
  await wait(1400)
  await cue({ chapter: '03 / STRUCTURE SCAN', title: '从外形进入完整装配关系', subtitle: '展开结构，再重新组装为可理解的数字终端', truth: '26 MESHES / OFFLINE DIGITAL MODEL', progress: 29, fullscreen: false })
  ...
  for (const [index, title, subtitle, progress] of [
    [0, '可靠感知每一个关节', '编码器、电流与独立 IMU 形成设备状态输入', 40],
    [2, '四路 CAN 汇聚分布式节点', '关节状态帧与 USB IMU 在机载计算侧汇合', 44],
    [4, '让状态进入智慧服务', '规则诊断连接任务调整、维护工单与复检归档', 49],
  ]) {
    await click(layerButtons.nth(index))
    await cue({ chapter: '04 / FIVE-LAYER SYSTEM', title, subtitle, truth: '输入 → 机制 → 输出 / 可解释', progress, fullscreen: false })
    await wait(3100)
  }

  // 47–70 / Real service state machine
  await cue({ chapter: '05 / SMART SERVICE DEMO', title: '一次可追溯的异常处置', subtitle: '以关节温升为例，演示从感知到服务归档的完整闭环', truth: '浏览器本地模拟数据 / 不连接实机', progress: 53, fullscreen: false })
  ...
  await cue({ chapter: '05 / RECHECK GATE', title: '复检未通过，系统拒绝归档', subtitle: '故障仍在时任务继续暂停，服务流程不能被跳过', truth: '状态机约束 / 失败路径验证', progress: 72, fullscreen: false })
  ...
  if (dryRun) {
    ...
    const silentPath = path.join(outputDir, '3S人形机器人_比赛展示视频_无音轨原片.mp4')
    const ffmpegArgs = ['-y', '-ss', trimStart.toFixed(3), '-i', rawPath, '-t', String(targetDuration), '-vf', 'fps=30,scale=1920:1080:flags=lanczos,format=yuv420p', '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-profile:v', 'high', '-level', '4.2', '-movflags', '+faststart', '-an', silentPath]
    await run(ffmpegPath, ffmpegArgs)
    for (const timestamp of [2, 18, 31, 55, 76, 87]) {
    ...
    const report = {
      ...
      durationTargetSeconds: targetDuration,
